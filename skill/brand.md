...
Eleven cool greys (`--n-0` … `--n-1000`) do about 90% of the work. One violet ramp
(`--accent-100` … `--accent-900`) is the accent and the only themeable channel. Magenta
(`--magenta-*`) and cyan (`--cyan-*`) exist **only** as chart series two and three — never in
chrome. Semantic hues are green / amber / red / blue, each with a 14–15% alpha tint for
surfaces (`--success-soft`, `--warning-soft`, `--danger-soft`, `--info-soft`).

Ink is full-opacity at every level. The system dims text by choosing a lighter grey
(`--text-primary` → `--text-secondary` → `--text-muted` → `--text-disabled`), never by
lowering opacity on type.

## Light theme

Light is a remap of the same tokens, not a second design. The neutral ramp reverses (`--n-0`
is always furthest from the ink, `--n-1000` is always the ink), surfaces become white paper on
...
Four flat surfaces stack: `--bg-sunken` (sidebar) → `--bg-app` (canvas) → `--surface-1` (cards)
→ `--surface-2` / `--surface-3` (inputs, menus, hovers). Elevation is a hairline border plus a
wide soft drop (`--shadow-md`), never a bright glow or heavy blur. Gradients appear in exactly
...
`--tracking-display`; UI runs 13–14px; 11px is reserved for uppercase section labels with
`--tracking-caps`. Line height 1.5 body, 1.15 display.

## Shape and border

Radii climb with the size of the thing: 4px checkbox, 6px chip, 8px control, 10px menu, 14px
card, 18px dialog, full for pills and avatars. Borders are always 1px: `--border-subtle`
separates structure, `--border-default` outlines interactive surfaces, `--border-strong` marks
...
use `--ease-out` with a 6px rise and a fade (`nocturne-rise`). Nothing bounces, nothing
...
0.5px down. Focus-visible is a 3px violet ring (`--ring-focus`) with the border switched to
`--accent-500`. Disabled is 45% opacity plus `not-allowed`. Rows and nav items follow the same
rules as buttons.
